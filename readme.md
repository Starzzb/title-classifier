...
长视频（≥120s）自动启用场景切分：先检测场景切换点，将视频切分为 ≤max_scenes 个场景段，逐段分析后合并描述。

### 处理流程

```
全片场景检测（HSV直方图差异，每 sample_interval 秒采样一帧）
  → 切换点缓存命中则跳过检测（按文件指纹+阈值+采样间隔键控）
  → 构建场景段（合并小场景直到 ≤ max_scenes）
  → 逐段并发分析（默认3路并发：抽帧+YOLO+每段一次VLM调用）
  → 合并各场景描述（最终一次VLM合并调用）
```
...
| 优化 | 说明 |
|------|------|
| 切换点缓存 | 首次检测后按文件指纹缓存到 `data/media.db` 的 `scene_cache` 表，重跑/重试时检测成本归零 |
| 逐段并发 | 场景段之间互相独立，`concurrent`（默认3）路并发处理；YOLO 推理自动串行（GPU锁），VLM API 并发路数=并发数 |
| 高分辨率降采样 | 检测时超过 1080p 的帧降采样后再算直方图（4K 视频显著加速，1080p 以下语义不变） |
| 采样间隔可调 | `sample_interval`（默认0.5s），长视频可增大到 1.0 换取更快检测 |

> **实测注记**：全片解码已达 libav 物理极限（~85x 实时），多线程/多进程并行解码无收益（cv2 ffmpeg 后端有全局锁，多 cap 并发为负加速）。场景模式的端到端大头在逐段分析（每段一次 VLM API 调用），并发优化正是针对此处。
...
[scene_detection]
enabled = true
threshold = 0.3           # 切换敏感度（0-1），越低越灵敏
max_scenes = 10           # 最大场景段数
frames_per_scene = 10     # 每段抽帧数
sample_interval = 0.5     # 检测采样间隔（秒）
concurrent = 3            # 逐段并发分析数
```
...
- 逐场景段并发分析（默认3路并发，`[scene_detection] concurrent` 可调），段序合并与串行一致
